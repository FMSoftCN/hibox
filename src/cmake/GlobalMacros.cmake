...
# HiBox), then put it there instead.

macro(HIBOX_COMPUTE_SOURCES _framework)
...
macro(HIBOX_INCLUDE_CONFIG_FILES_IF_EXISTS)
...
macro(HIBOX_ADD_SOURCE_DEPENDENCIES _source _deps)
    set(_tmp)
...
macro(HIBOX_ADD_PRECOMPILED_HEADER _header _cpp _source)
    if (MSVC)
        ...
        foreach (_src ${_sources})
            HIBOX_ADD_SOURCE_DEPENDENCIES(${_src} ${PrecompiledBinary})
        endforeach ()
...
macro(HIBOX_WRAP_SOURCELIST)
    foreach (_file ${ARGN})
        get_filename_component(_basename ${_file} NAME_WE)
        get_filename_component(_path ${_file} PATH)

        if (NOT _file MATCHES "${DERIVED_SOURCES_HIBOX_DIR}")
            string(REGEX REPLACE "/" "\\\\\\\\" _sourcegroup "${_path}")
            source_group("${_sourcegroup}" FILES ${_file})
        endif ()
    endforeach ()

    source_group("DerivedSources" REGULAR_EXPRESSION "${DERIVED_SOURCES_HIBOX_DIR}")
endmacro()

macro(HIBOX_FRAMEWORK_DECLARE _target)
...
macro(HIBOX_EXECUTABLE_DECLARE _target)
    add_executable(${_target} "${CMAKE_BINARY_DIR}/cmakeconfig.h")
endmacro()

# Private macro for setting the properties of a target.
# Rather than just having _target like HIBOX_FRAMEWORK and HIBOX_EXECUTABLE the parameters are
# split into _target_logical_name, which is used for variable expansion, and _target_cmake_name.
# This is done to support HIBOX_WRAP_EXECUTABLE which uses the _target_logical_name variables
# but requires a different _target_cmake_name.
macro(_HIBOX_TARGET _target_logical_name _target_cmake_name)
...
macro(_HIBOX_TARGET_ANALYZE _target)
    if (ClangTidy_EXE)
...
macro(HIBOX_FRAMEWORK _target)
    _HIBOX_TARGET(${_target} ${_target})
    _HIBOX_TARGET_ANALYZE(${_target})
...
# FIXME Move into HIBOX_FRAMEWORK after all libraries are using this macro
macro(HIBOX_FRAMEWORK_TARGET _target)
    ...
    add_library(HiBox::${_target} ALIAS ${_target}_PostBuild)
endmacro()

macro(HIBOX_EXECUTABLE _target)
    _HIBOX_TARGET(${_target} ${_target})
    _HIBOX_TARGET_ANALYZE(${_target})
...
macro(HIBOX_WRAP_EXECUTABLE _target)
    ...
    _HIBOX_TARGET(${_target} ${_wrapped_target_name})
    _HIBOX_TARGET_ANALYZE(${_wrapped_target_name})
...
macro(HIBOX_CREATE_FORWARDING_HEADER _target_directory _file)
    ...
    # HiBox2 generate-forwarding-headers script.
...
macro(HIBOX_CREATE_FORWARDING_HEADERS _framework)
    ...
    if (NOT WIN32)
        ...
        foreach (_currentArg ${ARGN})
            ...
            elseif (_processing_directories)
                ...
                foreach (_file ${_files})
                    HIBOX_CREATE_FORWARDING_HEADER(${_target_directory} ${_file})
                endforeach ()
            elseif (_processing_files)
                HIBOX_CREATE_FORWARDING_HEADER(${_target_directory} ${_currentArg})
            endif ()
        endforeach ()
    endif ()
endmacro()

function(HIBOX_MAKE_FORWARDING_HEADERS framework)
...
function(HIBOX_COPY_FILES target_name)
    set(options FLATTENED)
...
macro(HIBOX_DEBUG_DUMP_COMMANDS)
    set(CMAKE_VERBOSE_MAKEFILE ON)
endmacro()

macro(HIBOX_DEBUG_DUMP_VARIABLES)
...
macro(HIBOX_ADD_TARGET_PROPERTIES _target _property _flags)
...
macro(HIBOX_POPULATE_LIBRARY_VERSION library_name)
...
macro(HIBOX_CREATE_SYMLINK target src dest)
