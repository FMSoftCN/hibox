# The settings in this file are the HiBox project default values, and
...
# deviate from the defaults of the HiBox project (e.g. if the feature
# requires platform-specific implementation that does not exist).

set(_HIBOX_AVAILABLE_OPTIONS "")

set(PUBLIC YES)
set(PRIVATE NO)

macro(_ENSURE_OPTION_MODIFICATION_IS_ALLOWED)
    if (NOT _SETTING_HIBOX_OPTIONS)
        message(FATAL_ERROR "Options must be set between HIBOX_OPTION_BEGIN and HIBOX_OPTION_END")
    endif ()
endmacro()

macro(_ENSURE_IS_HIBOX_OPTION _name)
    list(FIND _HIBOX_AVAILABLE_OPTIONS ${_name} ${_name}_OPTION_INDEX)
    if (${_name}_OPTION_INDEX EQUAL -1)
        message(FATAL_ERROR "${_name} is not a valid HiBox option")
    endif ()
endmacro()

macro(HIBOX_OPTION_DEFINE _name _description _public _initial_value)
    _ENSURE_OPTION_MODIFICATION_IS_ALLOWED()

    set(_HIBOX_AVAILABLE_OPTIONS_DESCRIPTION_${_name} ${_description})
    set(_HIBOX_AVAILABLE_OPTIONS_IS_PUBLIC_${_name} ${_public})
    set(_HIBOX_AVAILABLE_OPTIONS_INITIAL_VALUE_${_name} ${_initial_value})
    set(_HIBOX_AVAILABLE_OPTIONS_${_name}_CONFLICTS "")
    set(_HIBOX_AVAILABLE_OPTIONS_${_name}_DEPENDENCIES "")
    list(APPEND _HIBOX_AVAILABLE_OPTIONS ${_name})

    EXPOSE_VARIABLE_TO_BUILD(${_name})
endmacro()

macro(HIBOX_OPTION_DEFAULT_PORT_VALUE _name _public _value)
    _ENSURE_OPTION_MODIFICATION_IS_ALLOWED()
    _ENSURE_IS_HIBOX_OPTION(${_name})

    set(_HIBOX_AVAILABLE_OPTIONS_IS_PUBLIC_${_name} ${_public})
    set(_HIBOX_AVAILABLE_OPTIONS_INITIAL_VALUE_${_name} ${_value})
endmacro()

macro(HIBOX_OPTION_CONFLICT _name _conflict)
    _ENSURE_OPTION_MODIFICATION_IS_ALLOWED()
    _ENSURE_IS_HIBOX_OPTION(${_name})
    _ENSURE_IS_HIBOX_OPTION(${_conflict})

    list(APPEND _HIBOX_AVAILABLE_OPTIONS_${_name}_CONFLICTS ${_conflict})
endmacro()

macro(HIBOX_OPTION_DEPEND _name _depend)
    _ENSURE_OPTION_MODIFICATION_IS_ALLOWED()
    _ENSURE_IS_HIBOX_OPTION(${_name})
    _ENSURE_IS_HIBOX_OPTION(${_depend})

    list(APPEND _HIBOX_AVAILABLE_OPTIONS_${_name}_DEPENDENCIES ${_depend})
endmacro()

macro(HIBOX_OPTION_BEGIN)
    set(_SETTING_HIBOX_OPTIONS TRUE)
...
# HIBOX_OPTION_DEFINE(ENABLE_XXX "Toggle XXX features" PUBLIC ON)
    HIBOX_OPTION_DEFINE(ENABLE_API_TESTS "Enable public API unit tests" PUBLIC OFF)
    HIBOX_OPTION_DEFINE(USE_SYSTEM_MALLOC "Toggle system allocator instead of HiBox's custom allocator" PRIVATE ${USE_SYSTEM_MALLOC_DEFAULT})
endmacro()

macro(_HIBOX_OPTION_ENFORCE_DEPENDS _name)
    foreach (_dependency ${_HIBOX_AVAILABLE_OPTIONS_${_name}_DEPENDENCIES})
        if (NOT ${_dependency})
...
macro(_HIBOX_OPTION_ENFORCE_ALL_DEPENDS)
    set(_OPTION_CHANGED TRUE)
    while (${_OPTION_CHANGED})
        set(_OPTION_CHANGED FALSE)
        foreach (_name ${_HIBOX_AVAILABLE_OPTIONS})
            if (${_name})
                _HIBOX_OPTION_ENFORCE_DEPENDS(${_name})
            endif ()
        endforeach ()
    endwhile ()
endmacro()

macro(_HIBOX_OPTION_ENFORCE_CONFLICTS _name)
    foreach (_conflict ${_HIBOX_AVAILABLE_OPTIONS_${_name}_CONFLICTS})
        if (${_conflict})
...
macro(_HIBOX_OPTION_ENFORCE_ALL_CONFLICTS)
    foreach (_name ${_HIBOX_AVAILABLE_OPTIONS})
        if (${_name})
            _HIBOX_OPTION_ENFORCE_CONFLICTS(${_name})
        endif ()
    endforeach ()
endmacro()

macro(HIBOX_OPTION_END)
    set(_SETTING_HIBOX_OPTIONS FALSE)

    list(SORT _HIBOX_AVAILABLE_OPTIONS)
    ...
    foreach (_name ${_HIBOX_AVAILABLE_OPTIONS})
        ...
        option(${_name} "${_HIBOX_AVAILABLE_OPTIONS_DESCRIPTION_${_name}}" ${_HIBOX_AVAILABLE_OPTIONS_INITIAL_VALUE_${_name}})
        if (NOT ${_HIBOX_AVAILABLE_OPTIONS_IS_PUBLIC_${_name}})
    ...
    _HIBOX_OPTION_ENFORCE_ALL_DEPENDS()
    _HIBOX_OPTION_ENFORCE_ALL_CONFLICTS()
...
macro(PRINT_HIBOX_OPTIONS)
    message(STATUS "Enabled features:")

    set(_should_print_dots ON)
    foreach (_name ${_HIBOX_AVAILABLE_OPTIONS})
        if (${_HIBOX_AVAILABLE_OPTIONS_IS_PUBLIC_${_name}})
            string(LENGTH ${_name} _name_length)
            set(_message " ${_name} ")
...
set(_HIBOX_CONFIG_FILE_VARIABLES "")

macro(EXPOSE_VARIABLE_TO_BUILD _variable_name)
    list(APPEND _HIBOX_CONFIG_FILE_VARIABLES ${_variable_name})
endmacro()
...
macro(HIBOX_CHECK_HAVE_INCLUDE _variable _header)
    check_include_file(${_header} ${_variable}_value)
    SET_AND_EXPOSE_TO_BUILD(${_variable} ${${_variable}_value})
endmacro()

macro(HIBOX_CHECK_HAVE_FUNCTION _variable _function)
    check_function_exists(${_function} ${_variable}_value)
    SET_AND_EXPOSE_TO_BUILD(${_variable} ${${_variable}_value})
endmacro()

macro(HIBOX_CHECK_HAVE_SYMBOL _variable _symbol _header)
    check_symbol_exists(${_symbol} "${_header}" ${_variable}_value)
    SET_AND_EXPOSE_TO_BUILD(${_variable} ${${_variable}_value})
endmacro()

macro(HIBOX_CHECK_HAVE_STRUCT _variable _struct _member _header)
    check_struct_has_member(${_struct} ${_member} "${_header}" ${_variable}_value)
    SET_AND_EXPOSE_TO_BUILD(${_variable} ${${_variable}_value})
endmacro()

macro(HIBOX_CHECK_SOURCE_COMPILES _variable _source)
